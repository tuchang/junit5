...
/**
* {@link PackageNameFilter} that matches fully qualified package names that are prefixed by one of
* the package names provided to the filter.
*
* <p>If the fully qualified name of a package starts with at least one of the packages names of the
* filter, the package will be included.
*
* @since 1.0
*/
class IncludePackageNameFilter implements PackageNameFilter {
	...
	private String formatExclusionReason(String packageName) {
		return String.format(
				"Package name [%s] does not match any included names: %s", packageName, patternDescription);
	}
