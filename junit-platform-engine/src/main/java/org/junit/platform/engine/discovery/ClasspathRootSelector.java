...
/**
* A {@link DiscoverySelector} that selects a <em>classpath root</em> so that {@link
* org.junit.platform.engine.TestEngine TestEngines} can search for class files or resources within
* the physical classpath &mdash; for example, to scan for test classes.
*
* <p>Since {@linkplain org.junit.platform.engine.TestEngine engines} are not expected to modify the
* classpath, the classpath root represented by this selector must be on the classpath of the
