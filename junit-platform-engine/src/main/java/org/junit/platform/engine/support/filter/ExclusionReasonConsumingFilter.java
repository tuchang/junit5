...
/**
* Decorator for a {@link Filter} that passes the <em>object</em> and the <em>reason</em> to a
* {@link BiConsumer} in case it is <em>excluded</em>.
*
* @since 1.0
*/
@API(Internal)
public class ExclusionReasonConsumingFilter<T> implements Filter<T> {
	...
	/**
	* Create a new {@code ExclusionReasonConsumingFilter} using the supplied {@code filter} and
	* {@code reasonConsumer}.
	*
	...
	*/
	public ExclusionReasonConsumingFilter(
			Filter<T> filter, BiConsumer<T, Optional<String>> reasonConsumer) {
