...
/**
* {@link PackageNameFilter} that matches fully qualified package names that are <em>not</em>
* prefixed by one of the package names provided to the filter.
*
* <p>If the fully qualified name of a package starts with at least one of the packages names of the
* filter, the package will be excluded.
*
* @since 1.0
*/
class ExcludePackageNameFilter implements PackageNameFilter {
	...
	ExcludePackageNameFilter(String... packageNames) {
		...
		Preconditions.containsNoNullElements(
				packageNames, "packageNames must not contain null elements");
	...
	private String formatInclusionReason(String packageName) {
		return String.format(
				"Package name [%s] does not match any excluded names: %s", packageName, patternDescription);
	}
	...
	private Optional<String> findMatchingName(String packageName) {
		return this.packageNames
				.stream()
				.filter(name -> name.equals(packageName) || packageName.startsWith(name + "."))
				.findAny();
	}
