...
/**
* This {@code Extension} provides native support for subclasses of the {@link ExternalResource}
* rule from JUnit 4.
*
* <p>{@code @Rule}-annotated fields as well as methods are supported.
*
* <p>By using this class-level extension on a test class such {@code ExternalResource}
* implementations in legacy code bases can be left unchanged including the JUnit 4 rule import
* statements.
*
...
public class ExternalResourceSupport implements BeforeEachCallback, AfterEachCallback {
	...
	private final Function<TestRuleAnnotatedMember, AbstractTestRuleAdapter> adapterGenerator =
			ExternalResourceAdapter::new;

	private final TestRuleFieldSupport fieldSupport =
			new TestRuleFieldSupport(this.adapterGenerator, ExternalResource.class);
	private final TestRuleMethodSupport methodSupport =
			new TestRuleMethodSupport(this.adapterGenerator, ExternalResource.class);
