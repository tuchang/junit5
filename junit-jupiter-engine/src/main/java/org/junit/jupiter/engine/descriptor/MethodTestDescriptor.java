...
*
* <p>The default display name for a test method is the name of the method concatenated with a
...
public class MethodTestDescriptor extends MethodBasedTestDescriptor {
	...
	protected ExtensionRegistry populateNewExtensionRegistry(JupiterEngineExecutionContext context) {
		return populateNewExtensionRegistryFromExtendWith(
	...
	private <T extends Extension> void invokeBeforeMethodsOrCallbacksUntilExceptionOccurs(
			...
			BiFunction<TestExtensionContext, T, Executable> generator,
			Class<T> type) {
				...
				() -> {
					try {
						...
						invokeTestExecutionExceptionHandlers(
								context.getExtensionRegistry(), testExtensionContext, throwable);
					}
				});
	}

	private void invokeTestExecutionExceptionHandlers(
			ExtensionRegistry registry, TestExtensionContext context, Throwable ex) {

		invokeTestExecutionExceptionHandlers(
				ex, registry.getExtensions(TestExecutionExceptionHandler.class), context);
	}

	private void invokeTestExecutionExceptionHandlers(
			Throwable ex, List<TestExecutionExceptionHandler> handlers, TestExtensionContext context) {
	...
	private <T extends Extension> void invokeAllAfterMethodsOrCallbacks(
			JupiterEngineExecutionContext context,
			BiFunction<TestExtensionContext, T, Executable> generator,
			Class<T> type) {
