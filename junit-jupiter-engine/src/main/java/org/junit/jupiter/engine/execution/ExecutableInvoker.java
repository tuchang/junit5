...
/**
* {@code ExecutableInvoker} encapsulates the invocation of a {@link java.lang.reflect.Executable}
* (i.e., method or constructor), including support for dynamic resolution of method parameters via
...
public class ExecutableInvoker {
	...
	* @param extensionRegistry the {@code ExtensionRegistry} to retrieve {@code ParameterResolvers}
	*     from
	*/
	public <T> T invoke(
	...
	/**
	* Invoke the supplied constructor with the supplied outer instance and dynamic parameter
	* resolution.
	*
	* <p>This method should only be used to invoke the constructor for an inner class.
	*
	* @param constructor the constructor to invoke and resolve parameters for
	* @param outerInstance the outer instance to supply as the first argument to the constructor
	* @param extensionContext the current {@code ExtensionContext}
	* @param extensionRegistry the {@code ExtensionRegistry} to retrieve {@code ParameterResolvers}
	*     from
	*/
	public <T> T invoke(
		...
		return ReflectionUtils.newInstance(
				constructor,
				resolveParameters(
						constructor, Optional.empty(), outerInstance, extensionContext, extensionRegistry));
	}
	...
	*/
	public Object invoke(
			Method method, ExtensionContext extensionContext, ExtensionRegistry extensionRegistry) {
	...
	*
	* @param method the method to invoke and resolve parameters for
	* @param target the object on which the method will be invoked; should be {@code null} for static
	*     methods
	* @param extensionContext the current {@code ExtensionContext}
	* @param extensionRegistry the {@code ExtensionRegistry} to retrieve {@code ParameterResolvers}
	*     from
	*/
	public Object invoke(
		...
		Optional<Object> optionalTarget =
				(target instanceof Optional ? (Optional<Object>) target : Optional.ofNullable(target));
		return ReflectionUtils.invokeMethod(
				method,
				target,
	...
	* @param target an {@code Optional} containing the target on which the executable will be
	*     invoked; never {@code null} but should be empty for static methods and constructors
	* @param extensionContext the current {@code ExtensionContext}
	* @param extensionRegistry the {@code ExtensionRegistry} to retrieve {@code ParameterResolvers}
	*     from
	* @return the array of Objects to be used as parameters in the executable invocation; never
	*     {@code null} though potentially empty
	*/
	...
	* @param target an {@code Optional} containing the target on which the executable will be
	*     invoked; never {@code null} but should be empty for static methods and constructors
	* @param outerInstance the outer instance that will be supplied as the first argument to a
	*     constructor for an inner class; should be {@code null} for methods and constructors for
	*     top-level or static classes
	* @param extensionContext the current {@code ExtensionContext}
	* @param extensionRegistry the {@code ExtensionRegistry} to retrieve {@code ParameterResolvers}
	*     from
	* @return the array of Objects to be used as parameters in the executable invocation; never
	...
	*/
	private Object[] resolveParameters(
		...
		for (int i = start; i < parameters.length; i++) {
			...
			values[i] =
					resolveParameter(parameterContext, executable, extensionContext, extensionRegistry);
		}
		return values;
	}
	...
	private Object resolveParameter(
		...
		try {
			// @formatter:off
			List<ParameterResolver> matchingResolvers =
					extensionRegistry
							.stream(ParameterResolver.class)
			...
			if (matchingResolvers.isEmpty()) {
				throw new ParameterResolutionException(
						String.format(
								"No ParameterResolver registered for parameter [%s] in executable [%s].",
			...
			throw new ParameterResolutionException(
					String.format(
							"Failed to resolve parameter [%s] in executable [%s]",
							parameterContext.getParameter(), executable.toGenericString()),
					ex);
		}
	}
