...
/**
* Class-level extension that turns on full logging (i.e., {@link Level#ALL}) for all specified
* classes for the duration of the current test container.
*
* <h3>Example Usage</h3>
*
* <pre class="code">
...
public @interface FullLogging {
	...
	class Extension implements BeforeAllCallback, AfterAllCallback {
		...
		public void beforeAll(ContainerExtensionContext context) throws Exception {
			...
			// @formatter:off
			Class<?>[] loggerClasses =
					findAnnotation(testClass, FullLogging.class)
							.orElseThrow(
									() ->
											new PreconditionViolationException(
													"@FullLogging must be declared on class " + testClass.getName()))
							.value();
		...
		public void afterAll(ContainerExtensionContext context) throws Exception {
			previouslyActiveLogLevels.forEach(
					(loggerName, previousLogLevel) ->
							Logger.getLogger(loggerName).setLevel(previousLogLevel));
		}
	}
}
