...
/**
* Factory methods for creating {@link PostDiscoveryFilter PostDiscoveryFilters} based on
* <em>included</em> and <em>excluded</em> tags.
*
* @since 1.0
*/
@API(Experimental)
public final class TagFilter {
	...
	*
	* <p>Containers and tests will only be executed if they are tagged with at least one of the
	* supplied <em>included</em> tags.
	*
	...
	*/
	public static PostDiscoveryFilter includeTags(List<String> tags) {
		...
		return descriptor ->
				FilterResult.includedIf(trimmedTagsOf(descriptor).anyMatch(tags::contains));
	}

	/**
	* Create an <em>exclude</em> filter based on the supplied {@code tags}.
	*
	* <p>Containers and tests will only be executed if they are <em>not</em> tagged with any of the
	* supplied <em>excluded</em> tags.
	*
	...
	*/
	public static PostDiscoveryFilter excludeTags(List<String> tags) {
		...
		Preconditions.containsNoNullElements(tags, "individual tags must not be null");
		return descriptor ->
				FilterResult.includedIf(trimmedTagsOf(descriptor).noneMatch(tags::contains));
	}
