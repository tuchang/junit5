...
/**
* The {@code LauncherDiscoveryRequestBuilder} provides a light-weight DSL for generating a {@link
...
public final class LauncherDiscoveryRequestBuilder {
	...
	*
	* <p>The {@code filters} are combined using AND semantics, i.e. all of them have to include a
	* resource for it to end up in the test plan.
	*
	* <p><strong>Warning</strong>: be cautious when registering multiple competing {@link
	* EngineFilter#includeEngines include} {@code EngineFilters} or multiple competing {@link
	* EngineFilter#excludeEngines exclude} {@code EngineFilters} for the same discovery request since
	* doing so will likely lead to undesirable results (i.e., zero engines being active).
	*
	...
	*
	* @param key the configuration parameter key under which to store the value; never {@code null}
	*     or blank
	...
	*
	* @param configurationParameters the map of configuration parameters to add; never {@code null}
	* @return this builder for method chaining
	* @see #configurationParameter(String, String)
	*/
	public LauncherDiscoveryRequestBuilder configurationParameters(
	...
	private void storeFilter(Filter<?> filter) {
		if (filter instanceof EngineFilter) {
			...
			throw new PreconditionViolationException(
					String.format(
							"Filter [%s] must implement %s, %s, or %s.",
							filter,
							EngineFilter.class.getSimpleName(),
							PostDiscoveryFilter.class.getSimpleName(),
							DiscoveryFilter.class.getSimpleName()));
		}
	}

	/** Build the {@link LauncherDiscoveryRequest} that has been configured via this builder. */
	public LauncherDiscoveryRequest build() {
		LauncherConfigurationParameters launcherConfigurationParameters =
