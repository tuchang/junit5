...
class DefaultLauncher implements Launcher {
	...
	private final TestExecutionListenerRegistry listenerRegistry =
			new TestExecutionListenerRegistry();
	...
	*/
	DefaultLauncher(Iterable<TestEngine> testEngines) {
		Preconditions.condition(
				testEngines != null && testEngines.iterator().hasNext(),
				() ->
	...
	public void execute(LauncherDiscoveryRequest discoveryRequest) {
		Preconditions.notNull(discoveryRequest, "LauncherDiscoveryRequest must not be null");
		execute(
				discoverRoot(discoveryRequest, "execution"), discoveryRequest.getConfigurationParameters());
	}
	...
	private void execute(Root root, ConfigurationParameters configurationParameters) {
		...
		TestExecutionListener testExecutionListener =
				this.listenerRegistry.getCompositeTestExecutionListener();
